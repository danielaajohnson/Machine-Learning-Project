# -*- coding: utf-8 -*-
"""Machine Learning Final - Bayes Implementation.ipynb
...
## I originally utilized SMOTE right here
## I took it out and put it inside the for loop so the data would be balanced for each training size
## I included the commented out lines below however so that you can see how I got the model nodes that are used within the for loop

#combined_data_balanced = pd.concat([X_train_balanced, y_train_balanced], axis=1)
#combined_data_balanced.to_excel('balanced_data.xlsx', index=False)

#estimator = HillClimbSearch(combined_data_balanced)
#model_structure = estimator.estimate(scoring_method=BicScore(combined_data_balanced))
...
for test_size in split_sizes:
    ...
    # this addresses the error I discuss in the paper (column names did not match becuase of one-hot encoding)
